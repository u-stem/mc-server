...
| タブ | 機能 |
|------|------|
| 概要 | 接続情報、プレイヤー数、リソース使用状況、TPS監視 |
| コンソール | サーバーログの確認（色分け表示）、コマンド実行 |
| プレイヤー | ホワイトリスト管理 |
| Mod / プラグイン | アップロード、有効/無効切り替え、Modrinthからインストール |
| バックアップ | ワールドデータのバックアップ・復元・インポート |
| スケジュール | サーバーの自動起動・停止スケジュール設定 |
| オートメーション | Discord通知、自動バックアップ、ヘルスチェック、プラグイン更新チェック |
| 基本設定 | サーバー名、ポート、メモリ、プリセット設定 |
| サーバー設定 | server.properties の編集 |
| バージョン | サーバーバージョンの変更 |
| ヘルプ | コマンドリファレンス、接続手順 |
...
| 機能 | 説明 |
|------|------|
| Discord通知 | サーバー起動/停止、クラッシュ検出、ヘルスアラート、バックアップ完了をDiscordに通知 |
| 自動バックアップ | 日次/週次スケジュールでバックアップ、起動/停止時バックアップ、古いバックアップの自動削除 |
| ヘルスチェック | TPS/メモリ監視、閾値を下回ると自動再起動、クラッシュ検出と自動復旧 |
| プラグイン更新チェック | Modrinthと連携してプラグインの更新を定期チェック、Discord通知 |
